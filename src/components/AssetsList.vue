      ...
      <table id="assetsTable">
        ...
        <tbody>
          ...
          <tr v-for="asset in assetList" 
            ...
            <td data-label="Price">{{ asset.price | usd }}</td>
            <td class="chart-icon" v-if="!isMobile" @click.stop="toggleChart(asset)">
              <SimpleChart 
                :dataPoints="asset.prices" 
                :lineWidth="1"/>
            </td>
            <td data-label="Balance">{{ asset.balance }}</td>
            <td data-label="Share">{{ asset.share | percent }}</td>
            <td data-label="Value">{{ asset.balance * asset.price | usd }}</td>
...
<script>
  ...
  export default {
    ...
    async mounted () {
      this.assetList = await Promise.all(this.assets.map(
        async (asset) => {
          ...
          asset.price = priceResponse.slice(-1)[0].value;
